...
# USER CONFIG
# ------------------------------------------------------------
PROJECT_DIR = r"C:\AI_Project"
OUT_DIR = os.path.join(PROJECT_DIR, "output")
VIDEO_PATH = os.path.join(PROJECT_DIR, "videos", "Prank_Video.mp4")

DEPTH_ANYTHING_REPO = os.path.join(PROJECT_DIR, "Depth-Anything-V2")
DEPTH_CKPT_PATH = os.path.join(PROJECT_DIR, "models", "depth_anything_v2_vits.pth")
SIGN_WEIGHTS = os.path.join(PROJECT_DIR, "models", "traffic_sign_yolov8.pt")
...
# Safety helpers
# ------------------------------------------------------------
def ensure_exists(path: str, what="file/dir"):
...
def make_writer_any(path_base_no_ext, fps, size_wh):
    ...
    """
    os.makedirs(os.path.dirname(path_base_no_ext), exist_ok=True)
    W, H = size_wh

    avi_path = path_base_no_ext + ".avi"
    avi_fourcc = cv2.VideoWriter_fourcc(*"MJPG")
    avi_writer = cv2.VideoWriter(avi_path, avi_fourcc, fps, (W, H))
    ...
    mp4_path = path_base_no_ext + ".mp4"
    mp4_writer = None
    for code in ["mp4v", "avc1", "H264"]:
        fourcc = cv2.VideoWriter_fourcc(*code)
        w = cv2.VideoWriter(mp4_path, fourcc, fps, (W, H))
        if w.isOpened():
            print(f"[OK] MP4 writer: {mp4_path} codec={code} size={W}x{H} fps={fps:.2f}")
            mp4_writer = w
            break

    return avi_writer, mp4_writer, avi_path, (mp4_path if mp4_writer else None)
...
def main():
    ...
    writer_avi, writer_mp4, out_avi, out_mp4 = make_writer_any(
        os.path.join(OUT_DIR, f"output_annotated_30fps_{RUN_TS}"), fps_in, (W0, H0)
    )
    writer_rgb_avi, writer_rgb_mp4, out_rgb_avi, out_rgb_mp4 = make_writer_any(
        os.path.join(OUT_DIR, f"output_rgb_depth_30fps_{RUN_TS}"), fps_in, (W0 * 2, H0)
    )
    ...
    writer_bev_avi, writer_bev_mp4, out_bev_avi, out_bev_mp4 = make_writer_any(
        os.path.join(OUT_DIR, f"output_bev_30fps_{RUN_TS}"), fps_in, (bevW, bevH)
    )
